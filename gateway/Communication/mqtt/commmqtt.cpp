...
CommMQTT::CommMQTT(QJsonObject settings)
    : m_pubQos{DEFAULT_QOS},
      m_subQos{DEFAULT_QOS},
      m_retries{0},
      m_publish{false},
      m_subscribe{false},
      m_client{nullptr},
...
void CommMQTT::connectComm() {
    if (m_client == nullptr) {
        ...
        const std::string server_address = QString("%1://%2%3%4").arg(QString::fromStdString(protocol), QString::fromStdString(host), (port == -1) ? QString() : QString(":"), QString::number(port)).toStdString();
        ...
        try {
            ...
            qCritical() << "MQTT connection exception:" << e.what();
            return emit connectionFailed();
        }
    }
